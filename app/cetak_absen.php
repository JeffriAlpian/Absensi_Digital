...
$queryUpdateAlpha = "UPDATE absensi SET status = 'A' 
                     ...
                     AND tanggal < CURDATE()"; // Untuk tanggal yang telah lewat
...
if ($kelas != '') {
    $absensiQuery .= " AND s.id_kelas = '$kelas'";
}
...
$wali_nip = '........................';
if ($kelas != '') {
    $qWali = mysqli_query($conn, "SELECT nama_wali, nip_wali FROM wali_kelas WHERE id_kelas = '$kelas' LIMIT 1");
...
while ($siswa = mysqli_fetch_assoc($siswaResult)) {
    ...
    for ($i = 1; $i <= $jumlahHari; $i++) {
        ...
        if ($val == '') {
            ...
            if ($val == 'H') {
                ...
                $pdf->SetTextColor(0, 128, 0); // Hijau Telat
                $pdf->Cell(6, 6, 'HT', 1, 0, 'C');
                $pdf->SetTextColor(0, 0, 0);
                $countS++;
            } elseif ($val == 'S') {
    ...
    $pdf->Cell(5, 6, $countH, 1, 0, 'C');
    $pdf->Cell(5, 6, $countS, 1, 0, 'C');
    $pdf->Cell(5, 6, $countI, 1, 0, 'C');
    $pdf->Cell(5, 6, $countA, 1, 1, 'C');
...
$pdf->SetX(50); // Mulai setelah kolom Nama
...
$pdf->Ln(14); // Jarak untuk tanda tangan
$pdf->SetX(50);
$pdf->SetFont('Arial', 'B', 10);
