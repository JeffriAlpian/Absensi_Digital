...
$sql .= " ORDER BY nama ASC LIMIT 50"; // Batasi hasil
...
if ($result->num_rows == 0) {
    $output = "<tr><td colspan='" . ($user_role === 'admin' ? 7 : 6) . "' class='px-4 py-4 text-center text-gray-500'>Data guru tidak ditemukan.</td></tr>";
} else {
    // Helper class (salin dari guru.php jika perlu)
    $btn_info = "bg-cyan-500 hover:bg-cyan-600 focus:ring-cyan-400";
    $btn_warning = "bg-red-500 hover:bg-yellow-600 focus:ring-yellow-400";
    $btn_print = "bg-green-600 hover:bg-green-700 focus:ring-blue-500"; 

    while ($row = $result->fetch_assoc()) {
        $qr_file = "../assets/qr/{$row['nip']}.png"; // Sesuaikan path relatif
        $qr_src = file_exists($qr_file) ? str_replace('../', '', $qr_file) : 'assets/qr/default.png'; // Path untuk src

        $output .= '<tr class="hover:bg-gray-50">';
        $output .= '<td class="px-4 py-2 whitespace-nowrap text-sm text-gray-700">' . htmlspecialchars($row['nip']) . '</td>';
        $output .= '<td class="px-4 py-2 whitespace-nowrap text-sm font-medium text-gray-900">' . htmlspecialchars($row['nama']) . '</td>';
        $output .= '<td class="px-4 py-2 whitespace-nowrap text-sm font-medium text-gray-900">' . htmlspecialchars($row['jabatan']) . '</td>';
        ...
        $output .= '<a href="app/cetak_kartu_individu_guru.php?guru=' . $row['id'] .'" target="_blank" class="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-md shadow-sm text-white ' . $btn_print .'">Cetak</a>';
        // Kolom aksi hanya untuk admin
        if ($user_role === 'admin') {

            $output .= '  <a href="?page=guru&edit=' . $row['id'] . '" class="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-md shadow-sm text-white ' . $btn_info . '">Edit</a>';
            $output .= '  <a href="?page=guru&hapus=' . $row['id'] . '" class="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-md shadow-sm text-white ' . $btn_warning . '">Hapus</a>';
