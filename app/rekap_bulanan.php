...
// Ambil siswa hanya yang aktif (gunakan alias tabel dan sanitasi input)
$kelas_int = intval($kelas);
$siswaQuery = "SELECT s.*, k.id AS id_kelas FROM siswa s JOIN kelas k ON s.id_kelas = k.id WHERE s.status='aktif'";
if ($kelas != '') {
  $siswaQuery .= " AND s.id_kelas = '$kelas_int'";
}
$siswaQuery .= " ORDER BY s.nama";
$siswaResult = mysqli_query($conn, $siswaQuery);
// Ambil data absensi (setelah di-update oleh query di atas) dengan alias tabel yang benar dan sanitasi input
$absensi = [];
...
$absensiQuery = "SELECT a.*, s.nis, s.nama, k.nama_kelas, s.id_kelas FROM absensi a 
                 ...
                 WHERE MONTH(a.tanggal) = '$bulan_int' 
                   AND YEAR(a.tanggal) = '$tahun_int'
                   AND s.status='aktif'";
if ($kelas != '') {
  $absensiQuery .= " AND s.id_kelas = '$kelas_int'";
}
...
$wali_nip = '........................';
if ($kelas != '') {
  $qWali = mysqli_query($conn, "SELECT nama_wali, nip_wali, k.id AS id_kelas, k.nama_kelas FROM wali_kelas w JOIN kelas k ON w.id_kelas = k.id WHERE id_kelas = '$kelas' LIMIT 1");
  ...
  <div class="mt-10">
    <table class="w-full border-0 text-sm text-center">
      <tr class="border-0">
        ...
        $k = mysqli_fetch_assoc($kelasList); ?>

        <td class="w-1/2 border-0">
          <?= htmlspecialchars($tanggal_terakhir) ?><br>
          Wali Kelas <?= $kelas != '' ? htmlspecialchars($k['nama_kelas']) : '(Semua Kelas)' ?><br>
